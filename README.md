...
[![编译安卓 APK](https://github.com/taotao-river/wechat-auto-reply/actions/workflows/build-apk.yml/badge.svg)](https://github.com/taotao-river/wechat-auto-reply/actions/workflows/build-apk.yml)
...
不熟悉命令行者可直接双击仓库中的 `安装到Mac.command`（首次需右键选择「打开」）。
...
**三层频率限制。** 单会话冷却（默认 30 分钟）、单会话每日上限、
全局每小时上限。最后一层作为熔断，防止配置错误导致大量发送。

**随机延迟。** 默认 3–12 秒，避免即时回复这一显著的自动化特征。
