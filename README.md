...
**同时跑 Android + macOS**（最常见的组合，有个重复回复的坑）：[`docs/android-macos-combo.md`](docs/android-macos-combo.md)
...
**跨端去重。** 微信多端同时在线时，同一条消息安卓和 macOS 会各上报一次。
引擎按「会话身份」（归一化后的会话名）而不是各端的 `chat_id` 记账，
两端共享同一份冷却和每日额度；再加一个 120 秒的内容去重窗口兜底。
**同时跑安卓和 macOS 的话，这条是必需的**，否则对方会收到两条一样的回复。
